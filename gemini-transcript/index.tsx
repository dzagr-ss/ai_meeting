...
class VoiceNotesApp {
  ...
  constructor() {
    const apiKey = window.ENV?.API_KEY || window.prompt('Enter your Gemini API Key:');
    if (!apiKey) {
    ...
    try {
      ...
      this.mediaRecorder.onstop = () => {
        ...
        if (this.audioChunks.length > 0) {
          ...
          this.summarizeFullAudio(audioBlob);
        } else {
  ...
  private async getPolishedNote(): Promise<void> {
    try {
      ...
      const prompt = `Take this raw transcription and create a polished, well-formatted note. Please respond in English only.
  ...
  private async summarizeFullAudio(audioBlob: Blob) {
    try {
      console.log('[Summary] Starting full audio summary...');
      this.recordingStatus.textContent = 'Summarizing full audio...';
      // Get API key from window.ENV or prompt
      const apiKey = window.ENV?.API_KEY || window.prompt('Enter your Gemini API Key:');
      console.log('[Summary] Using API key:', apiKey ? '***' : '(none)');
      if (!apiKey) {
        this.recordingStatus.textContent = 'API key required for summary.';
        return;
      }
      const summary = await summarizeAudioFile(audioBlob, apiKey);
      console.log('[Summary] Received summary:', summary);
      let summaryDiv = document.getElementById('audioSummary');
      if (!summaryDiv) {
        summaryDiv = document.createElement('div');
        summaryDiv.id = 'audioSummary';
      ...
      summaryDiv.innerHTML = `<h3>Meeting Summary</h3><pre style="white-space: pre-wrap;">${summary}</pre>`;
      this.recordingStatus.textContent = 'Summary complete.';
    } catch (err) {
      console.error('[Summary] Error summarizing audio:', err);
      this.recordingStatus.textContent = 'Error summarizing audio.';
    }
  }
}

// --- Add summarizeAudioFile utility ---
export async function summarizeAudioFile(
  file: File | Blob,
  apiKey: string
): Promise<string> {
  console.log('[Summary] summarizeAudioFile called');
  // Read file as base64
  ...
  // Prompt for summary
  const prompt = 'Summarize the main points and action items from this meeting audio. Please respond in English only.';
  ...
  try {
    ...
    console.log('[Summary] Gemini API response:', response);
...
// --- Add AudioSummary React component ---
// Use window.ENV.API_KEY or prompt for API key if not found
...
if (typeof window !== 'undefined') {
  // Dynamically import React/ReactDOM for the summary section
  Promise.all([
    import('react'),
    import('react-dom'),
  ]).then(([React, ReactDOM]) => {
    ...
    const AudioSummary = () => {
      ...
      const handleFileChange = async (e) => {
        ...
        setLoading(true);
        try {
          console.log('[Summary] AudioSummary file selected:', file);
          const result = await summarizeAudioFile(file, getApiKey());
          setSummary(result);
          console.log('[Summary] AudioSummary summary result:', result);
        } catch (err) {
          setSummary('Error summarizing audio.');
          console.error('[Summary] AudioSummary error:', err);
        }
        setLoading(false);
      };

      return (
        React.createElement('div', { style: {marginTop: 32, padding: 16, border: '1px solid #ccc', borderRadius: 8}},
          React.createElement('h2', null, 'Summarize Full Meeting Audio'),
          React.createElement('input', { type: 'file', accept: 'audio/*', onChange: handleFileChange }),
          loading && React.createElement('p', null, 'Summarizing...'),
          summary && React.createElement('div', null,
            React.createElement('h3', null, 'Summary'),
            React.createElement('pre', { style: {whiteSpace: 'pre-wrap'} }, summary)
          )
        )
      );
    };
    // Mount AudioSummary below the main app
    const summaryDiv = document.createElement('div');
    document.body.appendChild(summaryDiv);
    if (typeof ReactDOMDefault.createRoot === 'function') {
      ReactDOMDefault.createRoot(summaryDiv).render(React.createElement(AudioSummary));
    } else if (typeof ReactDOMDefault.render === 'function') {
      ReactDOMDefault.render(React.createElement(AudioSummary), summaryDiv);
    } else {
      console.error('[Summary] Could not render AudioSummary: no render method found on ReactDOM.');
    }
  });
}
